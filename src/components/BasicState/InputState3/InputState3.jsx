...
function InputState3() {
  ...
  const onChangeHandler = (e) => {
    ...
    //함수형 업데이트
    //set에 새로운 값을 넣는게 아니라 setInputValue((이전 상태의 값(prev)) => 새로운 값) format
    //prev는 이 함수를 실행하는 시점의 가장 최신 상태를 보장함
    //따라서 항상 최신 상태를 기반으로 다음 상태를 업데이트하므로 안정적이다
    setInputValue((prev) => {
      //...prev는 이전 상태의 객체를 그대로 복사해서 불변성을 지킨다
      //[name]: value => 복사된 객체에서 현재 입력이 변경된 속성의 값만 새로 덮어쓴다
      return {
        ...prev,
        [name]: value, //이전 상태의 값에 새로운 값을 덮어씌운다
      };
    });
  };
